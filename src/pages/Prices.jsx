      ...
      {hasContent ? (
        <>
          <div className="prices-detail__actions">
            ...
            <button type="button" className="prices-detail__close" onClick={onClose}>
              Close
            </button>
          </div>
          <DetailGrid
            items={[
              ...
              ["Retail price", Number.isFinite(value) ? gbp.format(value) : "Unavailable"],
            ]}
          />
          <PlaceholderList
            title="Later data"
...
export default function Prices() {
  ...
  const [dismissedDerivedCellKey, setDismissedDerivedCellKey] = React.useState(null);
  const matrixScrollRef = React.useRef(null);
  const priceCellRefs = React.useRef(new Map());

  const derivedCell = React.useMemo(() => {
    if (rowScope?.type !== "product" || columnScope?.type !== "range") return null;
    return buildSelectedCell(rowScope.value, columnScope.value);
  }, [columnScope, rowScope]);

  const derivedCellKey = derivedCell ? `${derivedCell.productId}:${derivedCell.columnId}` : null;
  ...
  React.useEffect(() => {
    ...
    setDismissedDerivedCellKey((current) => (current === derivedCellKey ? current : null));
  }, [derivedCellKey]);

  const finalSelectedCell = selectedCell || (derivedCellKey && dismissedDerivedCellKey !== derivedCellKey ? derivedCell : null);
  ...
  const hasBroadFocus = Boolean(!hasFinalCellSelection && (rowScope || columnScope));
  const hasActiveState = Boolean(rowScope || columnScope || selectedCell || derivedCellKey);
  ...
  function columnMatchesSelection(column) {
    ...
    if (columnScope.type === "weaver") return column.supplier === columnScope.value;
    return true;
  }
  ...
  function isRelevantPriceCell(product, sectionName, column) {
    ...
    return rowMatchesSelection(product, sectionName) && columnMatchesSelection(column);
  }
  ...
  function isProductLabelRelevant(productId) {
    ...
    if (rowScope.type === "category") return getProductCategory(productId) === rowScope.value;
    return true;
  }

  function isCategoryLabelRelevant(categoryName) {
    if (hasFinalCellSelection) {
      return finalSelectedCell ? getProductCategory(finalSelectedCell.productId) === categoryName : false;
    }

    if (!rowScope) return true;
    if (rowScope.type === "category") return rowScope.value === categoryName;
    if (rowScope.type === "product") return getProductCategory(rowScope.value) === categoryName;
    return true;
  }

  function isSupplierLabelRelevant(supplier) {
    if (hasFinalCellSelection) {
      return finalSelectedCell ? getRangeSupplier(finalSelectedCell.columnId) === supplier : false;
    }

    if (!columnScope) return true;
    if (columnScope.type === "weaver") return columnScope.value === supplier;
    if (columnScope.type === "range") return getRangeSupplier(columnScope.value) === supplier;
    return true;
  }
  ...
  function isRangeLabelRelevant(columnId) {
    ...
    if (columnScope.type === "weaver") return getRangeSupplier(columnId) === columnScope.value;
    return true;
  }
  ...
  const debugCellLabel = finalSelectedCell ? `${finalSelectedCell.productId}/${finalSelectedCell.columnId}` : "none";
  ...
  React.useEffect(() => {
    ...
    const timeoutId = isDetailOpen ? window.setTimeout(keepCellVisible, 240) : null;
  ...
  function toggleSupplierFocus(supplier) {
    setColumnScope((current) =>
      current?.type === "weaver" && current.value === supplier ? null : { type: "weaver", value: supplier }
    );
  ...
  function toggleRangeFocus(columnId) {
    setColumnScope((current) =>
      current?.type === "range" && current.value === columnId ? null : { type: "range", value: columnId }
    );
    setSelectedCell(null);
    resetDerivedDismissal();
  }
  ...
  function toggleCategoryFocus(categoryName) {
    setRowScope((current) =>
      current?.type === "category" && current.value === categoryName ? null : { type: "category", value: categoryName }
    );
    setSelectedCell(null);
    resetDerivedDismissal();
  }
  ...
  function toggleProductFocus(productId) {
    setRowScope((current) =>
      current?.type === "product" && current.value === productId ? null : { type: "product", value: productId }
    );
    setSelectedCell(null);
    resetDerivedDismissal();
  }
  ...
  function selectCell(productId, columnId) {
    setRowScope(null);
    setColumnScope(null);
    setSelectedCell(buildSelectedCell(productId, columnId) || { productId, columnId });
    resetDerivedDismissal();
  }
  ...
  return (
    <div className="prices-page">
      <header className="prices-header prices-element">
        <div>
          <h2>Prices</h2>
          <p>Read-only supplier matrix with stable focus states for quick staff lookup.</p>
        </div>
        <div className="prices-header__controls">
          ...
          <div className="prices-scope-debug" aria-label="Temporary Prices selection debug">
            {scopeDebugText}
          </div>
          <button
            type="button"
      ...
      <div className={`prices-layout ${isDetailOpen ? "prices-layout--detail-open" : "prices-layout--detail-closed"}`}>
        <section
        ...
        >
          <div className="prices-matrix__scroll" ref={matrixScrollRef}>
            <table>
              <thead>
                <tr>
                  <th className="prices-corner-heading" scope="col">Items</th>
                  {supplierGroups.map((group) => {
                    const isFocused =
                      columnScope?.type === "weaver" && columnScope.value === group.supplier && !hasFinalCellSelection;
                    const isDimmed = (hasBroadFocus || hasFinalCellSelection) && !isSupplierLabelRelevant(group.supplier);
                    return (
                      <th
                        key={group.supplier}
                        ...
                        scope="colgroup"
                      >
                        <button type="button" onClick={() => toggleSupplierFocus(group.supplier)}>
                  ...
                  {supplierGroups.flatMap((group) => (
                    group.columns.map((column) => {
                      const supplierFocused =
                        columnScope?.type === "weaver" && columnScope.value === column.supplier && !hasFinalCellSelection;
                      const rangeFocused =
                        columnScope?.type === "range" && columnScope.value === column.id && !hasFinalCellSelection;
                      const rangeDimmed = (hasBroadFocus || hasFinalCellSelection) && !isRangeLabelRelevant(column.id);
                      ...
                      return (
                        <th
                          ...
                          scope="col"
                        >
                          <button type="button" onClick={() => toggleRangeFocus(column.id)}>
              ...
              <tbody>
                {themedSections.map((section) => {
                  const categoryFocused =
                    ...
                    (rowScope?.type === "category" && rowScope.value === section.name ||
                      rowScope?.type === "product" && getProductCategory(rowScope.value) === section.name);
                  const categoryDimmed = (hasBroadFocus || hasFinalCellSelection) && !isCategoryLabelRelevant(section.name);
                  ...
                  return (
                    <React.Fragment key={section.name}>
                      ...
                      >
                        <th className="prices-category-merged-cell" colSpan={priceColumns.length + 1} scope="rowgroup">
                          <button
                            type="button"
                      ...
                      {section.products.map((product) => {
                        const productFocused =
                          rowScope?.type === "product" && rowScope.value === product.id && !hasFinalCellSelection;
                        const productDimmed = (hasBroadFocus || hasFinalCellSelection) && !isProductLabelRelevant(product.id);
                        ...
                        return (
                          <tr
                            ...
                            className={[
                              ...
                              productSelectionDimmed ? "is-selection-dimmed" : "",
                            ].filter(Boolean).join(" ")}
                            style={section.themeStyle}
                          >
                            <th className="prices-product-cell" scope="row">
                              <button type="button" onClick={() => toggleProductFocus(product.id)}>
                                {product.name}
                              </button>
                            </th>
                            ...
                            {priceColumns.map((column) => {
                              ...
                              const isSelectedCell = isSelectedFinalCell(product.id, column.id);
                              const isImpliedCell =
                                !selectedCell && isSelectedCell;
                              const cellKey = `${product.id}:${column.id}`;
                              const isRelevantCell = isRelevantPriceCell(product, section.name, column);
                              const selectionDimmed = hasActiveState && !isSelectedCell && !isRelevantCell;
                              ...
                              return (
                                <td
                                  ...
                                  ref={(element) => {
                                    if (element) {
                                      priceCellRefs.current.set(cellKey, element);
                                    } else {
                                ...
                                >
                                  {available ? (
                                    <button type="button" onClick={() => selectCell(product.id, column.id)}>
                                      {gbp.format(value)}
                                    </button>
                                  ) : (
                                    <button
                                      ...
                                      onClick={() => selectCell(product.id, column.id)}
                                    >
                                      -
                                    </button>
                                  )}
        ...
        <DetailPanel selectedCell={detailCell} onClose={clearSelectedCell} isOpen={isDetailOpen} />
      </div>
    </div>
  );
}
