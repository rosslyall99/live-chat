...
serve(async (req) => {
  ...
  try {
    ...
    // ----------------------------
    // 1) Conversations created in range (claimed + first reply)
    //    Claimed attribution is by assigned_to.
    // ----------------------------
    let createdQ = admin
      .from("conversations")
      .select("id, site_id, assigned_to, handled_by, status, created_at, last_message_at, closed_at")
    ...
    // ----------------------------
    // 2) Conversations closed in range (closure + duration)
    //    Closure attribution is by handled_by (who actually closed it).
    // ----------------------------
    let closedQ = admin
      .from("conversations")
      .select("id, site_id, assigned_to, handled_by, status, created_at, closed_at")
      .eq("status", "closed")
    ...
    // ✅ When filtering by agent in Insights, filter by who closed it
    if (agent_id) closedQ = closedQ.eq("handled_by", agent_id);
    ...
    // Claimed + first reply (conversations CREATED in range; attributed by assigned_to)
    ...
    // Closed + duration (conversations CLOSED in range; attributed by handled_by)
    let unattributedClosedCount = 0;

    for (const c of closedList) {
      const uid = c.handled_by as string | null;

      if (!uid) {
        unattributedClosedCount += 1;
        continue;
      }
    ...
    // ✅ Add synthetic "Unattributed" row (only when not filtering by a specific agent)
    if (!agent_id && unattributedClosedCount > 0) {
      agents.push({
        ...
        claimed_count: 0,
        closed_count: unattributedClosedCount,

        avg_first_reply_seconds: null,
        avg_chat_duration_minutes: null,
      });
    }
    ...
    const overall = {
      ...
      closed_unattributed: unattributedClosedCount,
    };
